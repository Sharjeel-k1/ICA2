...
(defn shortest-path [g start destination edge-distance-function max-distance max-edges]
  ...
  (let [vertex-distance (make-vertex-distance)
    ...
    (loop []
      (let [vertex-a-name (minimum-distance-vertex g vertex-distance not-completed-vertex-set)
    ...
    (when (some? @shortest-path-found)
      (println (str "Shortest path(s) from " start " to " destination " found with price:" (vertex-distance-get vertex-distance destination)))
...
; -------- MODIFY start, destination, and trip type
(defn -main []
  ...
  (cond
    (= *trip-type* "family")
    (shortest-path g *departure* *destination* problem-1-edge-distance-function 700 2)

    (= *trip-type* "organized")
    (shortest-path g *departure* *destination* problem-2-edge-distance-function 1000 3)
